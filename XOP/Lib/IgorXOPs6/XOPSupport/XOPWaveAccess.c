...
/*	WaveNote(waveH)
	...
	Thread Safety: WaveNote is Igor-thread-safe with XOP Toolkit 6 and Igor Pro 6.20 or later
	except for waves passed to threads as parameters. You can call it from a thread created
	by Igor but not from a private thread that you created yourself.
*/
Handle
...
/*	WaveTextEncoding(waveH, waveElementBitMask, getEffectiveTextEncoding, tecPtr)

	Returns via tecPtr a code that identifies the text encoding of the stored
	text associated with the part of a wave identified by waveElementBitMask.
	
	waveElementBitMask is one of the following:
		1:	Wave name
		2:	Wave units
		4:	Wave note
	...
	The values returned via tecPtr are described in the documenation for the built-in
	Igor WaveTextEncoding function in Igor Pro 7. The most common values are:
		1: UTF-8
		2: MacRoman
		3: Windows-1252
		4: Shift-JIS
	If the a text wave is marked as containing binary data rather than text data,
	the value returned will be 255. See the documentation for the built-in Igor7
	WaveTextEncoding function for details.
	
	Added for Igor Pro 7.00D00, 2012-11-25. If you call this with an earlier version
	of Igor, it will return IGOR_OBSOLETE and do nothing.
*/
int
WaveTextEncoding(waveHndl waveH, int waveElementBitMask, int getEffectiveTextEncoding, int* tecPtr)
{
	return (int)CallBack4(WAVE_TEXT_ENCODING, waveH, (void*)waveElementBitMask, (void*)getEffectiveTextEncoding, tecPtr);
}
